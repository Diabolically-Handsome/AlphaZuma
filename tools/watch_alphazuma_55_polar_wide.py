"""Launch and supervise the frozen wide polar run after DAgger exits."""
...
def run(plan_path: Path, expected_plan_sha256: str, poll_seconds: float) -> int:
    ...
    if _sha256(plan_path) != expected_plan_sha256:
        raise ValueError("wide launch plan hash differs")
    plan = _read_json(plan_path)
    if (
        plan.get("schema") != "zuma-rl.alphazuma-55-polar-wide-launch-plan"
        or plan.get("version") != 1
        or plan.get("status") != "FROZEN_BEFORE_WAIT"
    ):
        raise ValueError("unexpected wide launch plan")
    ...
    preregistration = _bound(plan["wide_preregistration"], "wide preregistration")
    _bound(plan["source_dagger_preregistration"], "DAgger preregistration")
    trainer = _bound(plan["implementation"]["trainer"], "wide trainer")
    ...
    source_pid = int(source["trainer_pid_at_registration"])
    state: dict[str, Any] = {
        "schema": "zuma-rl.alphazuma-55-polar-wide-watcher-status",
        "version": 1,
        "status": "RUNNING",
        "stage": "WAITING_FOR_DAGGER_TERMINAL",
        "started_utc": _utc_now(),
        "updated_utc": _utc_now(),
        "plan": {"path": str(plan_path), "sha256": expected_plan_sha256},
        "source_trainer_pid": source_pid,
    ...
    try:
        while not source_completion.exists() and not source_failure.exists():
            if not _pid_alive(source_pid):
                raise RuntimeError("DAgger trainer exited without a terminal receipt")
        ...
        if source_receipt.get("status") not in {"COMPLETE", "FAILED"}:
            raise RuntimeError("DAgger terminal receipt has an invalid status")
        ...
        if return_code != 0 or not target_completion.exists():
            raise RuntimeError(
                f"wide trainer failed with code {return_code}; failure={target_failure}"
            )
        completion = _read_json(target_completion)
        if completion.get("status") != "COMPLETE":
            raise RuntimeError("wide completion receipt is not COMPLETE")
        state.update(
            {
                "status": "COMPLETE",
                "stage": "COMPLETE",
