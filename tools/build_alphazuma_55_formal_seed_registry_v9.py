"""Extend the formal seed registry for the head-only conditional successor."""
...
PREDECESSOR_SHA256 = (
    "sha256:0f3af8fbb7758d82a413f6e27101d04feb94925ffc2effe7b963f6e55656e43b"
)
MASTER = PROJECT_ROOT / (
...
CAMPAIGN_ID = "motor-observable-gradual-headonly-successor-s99081644-v1"
...
def build(*, expected_master_sha256: str, output: Path) -> dict[str, Any]:
    ...
    if not (
        ...
        == [3_900_000_000, 3_900_000_439]
        and [int(continuous["first"]), int(continuous["last"])]
        == [4_000_000_000, 4_000_000_219]
    ):
        raise ValueError("head-only successor seed contract differs")
    campaigns = list(predecessor.get("campaigns", []))
    if any(str(row.get("id")) == CAMPAIGN_ID for row in campaigns):
        raise ValueError("head-only successor already registered")
    campaigns.append(
        {
            "id": CAMPAIGN_ID,
            "master": master_ref,
