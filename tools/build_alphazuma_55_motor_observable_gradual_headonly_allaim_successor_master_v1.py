"""Freeze the conditional formal master for the factorial route."""
...
from tools import (
    build_alphazuma_55_motor_observable_gradual_headonly_successor_master_v1
    as legacy,
)
from tools import (
...
def build(*, expected_plan_sha256: str, output: Path) -> dict[str, Any]:
    ...
    if output.exists():
        raise FileExistsError(f"factorial successor master exists: {output}")
    ...
    try:
        ...
        legacy._write_new = lambda _path, value: captured.setdefault(
    ...
    master.update(
        {
            ...
            "deadline_utc": "2026-08-16T20:00:00Z",
            "deadline_local": "2026-08-16 16:00 America/Toronto",
        }
    )
    master["objective"]["target"] = (
        "Certify one frozen head-only/all-actions-aim policy that can clear "
        "all 55 included levels; speed is explicitly secondary."
    )
    ...
    master["implementation"].update(
        {
            ...
            "independent_auditor": _reference(
                ...
                "headonly_allaim_successor_v1.py"
            ),
    ...
    master["outputs"].update(
        {
            ...
            "independent_audit_receipt": str(
                ...
                "headonly-allaim-successor-s99081649-independent-audit-v1.json"
            ),
        }
    )
    master["route_contract"] = {
        "causal_factorial": (
            "freeze_source_representation_train_action_net_only_and_"
            "supervise_all_action_aims"
        ),
