"""Run five-candidate replay-anchor certification with frozen tie semantics."""
...
def _load_master(path: Path) -> dict[str, Any]:
    ...
    for name, expected in expected_actual.items():
        ...
        base._require(
            ...
            f"replay-anchor successor {name} binding differs",
        )
    ...
    base._require(
        ...
        == EXPECTED_CANDIDATES,
        "replay-anchor successor compatibility declaration differs",
    )
    plan_path = Path(
    ...
    base._require(
        ...
        "replay-anchor actual engineering matrix differs",
    )
    ...
    def adapted_read(candidate: Path) -> dict[str, Any]:
        ...
        if resolved == path:
            ...
            value = copy.deepcopy(value)
            value["status"] = "FROZEN_BEFORE_TARGET_PREREGISTRATION"
        return value
