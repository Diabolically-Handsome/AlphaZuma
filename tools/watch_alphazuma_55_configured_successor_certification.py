"""Watch a configured successor controller and run its bound auditor."""
...
def run(
    ...
    if controller._sha256(master_path) != expected_master_sha256:
        raise ValueError("configured successor master hash differs")
    ...
    if not isinstance(watcher_reference, dict):
        raise ValueError("configured successor master does not bind the watcher")
    if (
        Path(str(watcher_reference.get("path", ""))).resolve(strict=True)
        != SCRIPT_PATH
        or watcher_reference.get("sha256") != controller._sha256(SCRIPT_PATH)
    ):
        raise ValueError("configured successor watcher binding differs")
    ...
    state: dict[str, Any] = {
        "schema": "zuma-rl.alphazuma-55-configured-successor-watcher-status",
        "version": 1,
        "status": "RUNNING",
    ...
    try:
        ...
        while True:
            ...
            if not _pid_alive(controller_pid):
                raise RuntimeError(
                    "configured successor controller exited without a terminal receipt"
                )
            time.sleep(poll_seconds)
        if controller_status is None or controller_status.get("status") == "FAILED":
            raise RuntimeError("configured successor controller reported FAILED")
        ...
        if audit_receipt.exists():
            ...
            if receipt.get("status") != "PASS":
                raise RuntimeError(
                    "existing configured successor audit receipt is not PASS"
                )
        else:
            command = [
                sys.executable,
            ...
            if result.returncode:
                raise RuntimeError(
                    f"configured successor independent auditor exited with {result.returncode}"
                )
            receipt = _read(audit_receipt)
            if receipt.get("status") != "PASS":
                raise RuntimeError("configured successor independent audit is not PASS")
        state.update(
            {
                "status": "COMPLETE",
                "phase": "COMPLETE",
