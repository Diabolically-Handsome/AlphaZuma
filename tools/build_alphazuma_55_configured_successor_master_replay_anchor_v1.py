"""Freeze five-candidate certification for replay-anchored DAgger."""
...
from tools.materialize_alphazuma_55_polar_intent_replay_anchor_validation import (
...
PLAN_BINDING_KEY = "polar_intent_replay_anchor_validation_plan"
EXPECTED_RANKING = [
    "wins_desc",
    "level_coverage_desc",
    "total_score_desc",
...
EXPECTED_CANDIDATES = [
    "polar-intent-replay-anchor-source-wide-final",
    "polar-intent-replay-anchor-source-dagger-final",
...
def build_master(
    *, successor_intent_path: Path, expected_intent_sha256: str
    ...
    base._require(
        ...
        "replay-anchor successor intent hash differs",
    )
    intent = base._read_json(successor_intent_path)
    base._require(
        intent.get("schema")
        == "zuma-rl.alphazuma-55-polar-intent-replay-anchor-successor-intent"
        and intent.get("status") == "FROZEN_BEFORE_ENGINEERING_RESULT",
        "unexpected replay-anchor successor intent",
    )
    ...
    base._require(
        ...
        "replay-anchor candidate order differs",
    )
    plan_path = Path(
    ...
    base._require(
        ...
        "replay-anchor engineering plan differs",
    )
    ...
    base._require(
        ...
        "replay-anchor formal registry snapshot changed",
    )
    ...
    base._require(
        ...
        is True,
        "replay-anchor prior formal registry is not clean",
    )
    ...
    master["implementation"].update(
        {
            ...
            "controller": _artifact(
                ...
                "certification_replay_anchor_v1.py"
            ),
            "watcher": _artifact(
                PROJECT_ROOT
    ...
    master["compatibility_adapter"] = {
        ...
        "base_builder_validation_projection": (
            "temporarily omit the first of two non-promotable source "
            "baselines and project 5x55 to 4x55 solely while exercising "
            "the legacy static builder"
        ),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if output.exists():
        raise FileExistsError(
            f"refusing to overwrite replay-anchor successor master: {output}"
        )
    master = build_master(
