"""Freeze the conditional formal master for all-actions V2 engineering."""
...
from tools import (
    run_alphazuma_55_motor_observable_gradual_allaim_successor_v2 as controller,
)
...
def build(*, expected_plan_sha256: str, output: Path) -> dict[str, Any]:
    ...
    if output.exists():
        raise FileExistsError(f"all-actions V2 successor master exists: {output}")
    ...
    try:
        ...
        legacy._write_new = (
            lambda _path, value: captured.setdefault("master", copy.deepcopy(value))
        )
        legacy.build(
    ...
    master["formal_seed_registry"]["active_registry_path"] = str(ACTIVE_REGISTRY)
    master["implementation"].update(
        {
            ...
            "engineering_controller": _reference(controller.engineering.SCRIPT_PATH),
    ...
    master["outputs"].update(
        {
            ...
            "independent_audit_receipt": str(
                PROJECT_ROOT
                / "diagnostics/alphazuma-55-motor-observable-gradual-allaim-"
                "successor-s99081636-independent-audit-v2.json"
            ),
        }
    )
    master["reader_lineage"] = {
        ...
        "formal_seed_ranges_changed": False,
    }
