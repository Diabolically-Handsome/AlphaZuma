"""Bind frozen polar checkpoints into their paired full55 validation."""
...
def validate_plan_static(plan_path: Path, expected_sha256: str) -> dict[str, Any]:
    ...
    _require(
        plan.get("schema") == "zuma-rl.alphazuma-55-polar-validation-plan"
        and plan.get("version") == 1
        and plan.get("status") == "FROZEN_DURING_TARGET_TRAINING",
        "unexpected polar validation plan",
    )
    ...
    _require(
        ...
        == EXPECTED_EPOCHS,
        "plan does not bind the frozen polar training contract",
    )
    _bound_path(plan["baseline"], "baseline model")
    _bound_path(plan["baseline"]["migration"], "baseline migration")
    ...
    _require(
        ...
        "invalid polar validation matrix",
    )
    _require(
        plan["authority_boundary"]["formal_seed_consumption"] is False
        and plan["authority_boundary"]["current_campaign_candidate_authority"]
        is False,
...
def materialize(plan_path: Path, expected_sha256: str) -> tuple[Path, Path]:
    ...
    completion_path = Path(plan["target"]["expected_completion"]).resolve(strict=True)
    ...
    _require(
        ...
        "polar training completion is invalid",
    )
    checkpoint_by_epoch = {
    ...
    _require(
        ...
        "polar completion misses a frozen checkpoint",
    )
    ...
    models = [
        {
            ...
            "path": str(_bound_path(baseline, "baseline model")),
            "sha256": baseline["sha256"],
        }
    ]
    expected_paths = tuple(Path(value).resolve() for value in plan["target"]["checkpoint_paths"])
    for epoch, expected_path in zip(EXPECTED_EPOCHS, expected_paths, strict=True):
        ...
        _require(path == expected_path, f"epoch {epoch} path differs from plan")
        digest = contracts._sha256(path)
        _require(row["sha256"] == digest, f"epoch {epoch} hash mismatch")
        models.append(
            {
                "id": f"polar-full55-epoch-{epoch:02d}",
    ...
    master_path = Path(plan["master_preregistration"]["path"]).resolve(strict=True)
    ...
    manifest["polar_validation_plan"] = plan_reference
    manifest["target_completion"] = completion_reference
    preregistration["polar_validation_plan"] = plan_reference
