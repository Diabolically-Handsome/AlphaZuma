"""Wait for polar training, then run its frozen full55 engineering validation."""
...
from tools.materialize_alphazuma_55_polar_validation import (
    materialize,
...
def run(
    *,
    ...
    if status_root.exists():
        raise FileExistsError(f"polar watcher status root exists: {status_root}")
    ...
    def status(stage: str, **extra: Any) -> None:
        _write_atomic(
            status_path,
            {
                "schema": "zuma-rl.alphazuma-55-polar-validation-watcher",
                "version": 1,
                "status": "RUNNING",
                "stage": stage,
    ...
    try:
        ...
        if completion.get("status") != "COMPLETE":
            raise ValueError("polar completion status is not COMPLETE")
        reset_audit_path = Path(
            plan["wait_for_engineering_slot"]["reset_aim_audit"]
        )
        while not reset_audit_path.exists():
            status(
                ...
                waiting_for=str(reset_audit_path),
            )
            time.sleep(poll_seconds)
        reset_audit = _read(reset_audit_path)
        if reset_audit.get("status") != "PASS":
            raise RuntimeError("reset-aim engineering audit did not PASS")
        ...
        with (status_root / "evaluator.stdout.log").open(
            ...
            if process.returncode != 0:
                raise RuntimeError(
                    f"polar evaluator exited with code {process.returncode}"
                )
        matrix = _matrix_snapshot(matrix_path)
        if (
            matrix is None
            or matrix["status"] != "COMPLETE"
            or int(matrix["completed_attempts"]) != 275
            or int(matrix["expected_attempts"]) != 275
        ):
            raise RuntimeError("polar validation matrix is not complete")
        ...
        if result.returncode != 0:
            raise RuntimeError(f"polar auditor exited with code {result.returncode}")
        audit = _read(audit_output)
        if audit.get("status") != "PASS":
            raise RuntimeError("polar independent audit did not PASS")
        final = {
            "schema": "zuma-rl.alphazuma-55-polar-validation-watcher",
            "version": 1,
            "status": "COMPLETE",
        ...
        failure = {
            "schema": "zuma-rl.alphazuma-55-polar-validation-watcher-failure",
            "version": 1,
            "status": "ERROR",
            "failed_utc": _utc_now(),
            "wall_seconds": time.perf_counter() - started,
