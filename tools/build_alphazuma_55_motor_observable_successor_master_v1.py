"""Freeze the conditional formal master for the motor-observable route."""
...
CAMPAIGN_ID = "alphazuma-55-motor-observable-successor-s99081618-v1"
EXPECTED_ENGINEERING_PLAN_SHA256 = (
    "sha256:5dad99563dfbb11d705d446daa4a83051100dc10094e67d5e46fa377115f0964"
)
EXPECTED_REGISTRY_V4_SHA256 = (
    "sha256:5d092dc366530e88c8d93e490731dc1597157260099c3a175f3f5b1070e3e57f"
)
...
def _write_new(path: Path, value: dict[str, Any]) -> None:
    ...
    if path.exists():
        temporary.unlink()
        raise FileExistsError(f"successor master already exists: {path}")
...
def build(*, project_root: Path, output: Path) -> dict[str, Any]:
    ...
    if controller._sha256(engineering_plan) != EXPECTED_ENGINEERING_PLAN_SHA256:
        raise ValueError("engineering plan bytes changed")
    ...
    if controller._sha256(registry_v4) != EXPECTED_REGISTRY_V4_SHA256:
        raise ValueError("formal seed registry V4 bytes changed")
    active_registry = (
        root
        / "diagnostics/alphazuma-55-weekend-formal-seed-registry-"
        "s99081619-v5.json"
    ...
    master = {
        ...
        "objective": {
            ...
            "target": (
                "Certify one promoted motor-observable policy on all 55 "
        ...
        "formal_seed_registry": {
            "predecessor": _reference(registry_v4),
        ...
        "implementation": {
            ...
            "controller": _reference(
                root
                / "tools/run_alphazuma_55_motor_observable_successor_v1.py"
            ),
        ...
        "outputs": {
            ...
            "independent_audit_receipt": str(
                root
                / "diagnostics/alphazuma-55-motor-observable-successor-"
                "s99081618-independent-audit-v1.json"
            ),
        },
        "authority_boundary": {
