"""Bind multiseed checkpoints into their frozen fresh validation."""
...
def validate_plan_static(plan_path: Path, expected_sha256: str) -> dict[str, Any]:
    _require(
        ...
        "multiseed validation plan hash mismatch",
    )
    plan = _read(plan_path)
    _require(
        plan.get("schema")
        == "zuma-rl.alphazuma-55-settled-v3-multiseed-validation-plan"
        and plan.get("version") == 1
        and plan.get("status") == "FROZEN_BEFORE_TARGET_MODEL",
        "unexpected multiseed validation plan",
    )
    ...
    _require(
        ...
        and int(distillation.get("run", {}).get("rounds", -1)) == 4,
        "plan does not bind four-round multiseed distillation",
    )
    ...
    _require(
        ...
        "invalid multiseed validation matrix",
    )
    _require(
        tuple(int(row["epoch"]) for row in plan["models"]["target"]["expected_checkpoints"])
        == EXPECTED_EPOCHS,
        "multiseed checkpoint schedule differs",
    )
    _require(
...
def materialize(plan_path: Path, expected_sha256: str) -> tuple[Path, Path]:
    ...
    aggregate = completion.get("aggregate_collection", {})
    _require(
        completion.get("schema")
        == "zuma-rl.alphazuma-55-settled-v3-multiseed-completion"
        and completion.get("status") == "COMPLETE"
        and int(aggregate.get("rounds", -1)) == 4
        and int(aggregate.get("episodes", -1)) == 220
        and int(aggregate.get("capacity_overflows", -1)) == 0,
        "multiseed distillation is incomplete or invalid",
    )
    checkpoint_by_epoch = {
    ...
    manifest["settled_v3_multiseed_validation_plan"] = plan_reference
    manifest["target_completion"] = completion_reference
    preregistration["settled_v3_multiseed_validation_plan"] = plan_reference
