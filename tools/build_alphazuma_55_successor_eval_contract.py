"""Build hash-bound successor final-blind or continuous evaluation contracts."""
...
def _validated_stage_spec(
    *, stage: str, base_seed: int, attempts_per_level: int
    ...
    if stage not in EXPECTED_ATTEMPTS:
        raise ValueError("successor stage must be final_blind or continuous")
    attempts = int(attempts_per_level)
    if attempts != EXPECTED_ATTEMPTS[stage]:
        raise ValueError(
            f"successor {stage} requires {EXPECTED_ATTEMPTS[stage]} attempts"
        )
    seed = int(base_seed)
    if not 0 <= seed <= (2**32 - 1):
        raise ValueError("successor seed is outside uint32")
    last = seed + 55 * attempts - 1
    if last > 2**32 - 1:
        raise ValueError("successor seed matrix exceeds uint32")
...
def build_successor_contracts(
    *,
    ...
    if (
        ...
        raise ValueError("unexpected successor master preregistration")
    ...
    if not models or len({str(model.get("id", "")) for model in models}) != len(
        ...
        raise ValueError("successor model ids must be non-empty and unique")
    for index, model in enumerate(models):
        ...
        if not model_id:
            raise ValueError(f"successor models[{index}] has an empty id")
        if int(model.get("training_steps", -1)) < 0:
            raise ValueError(
                f"successor models[{index}] has invalid training steps"
            )
        if model.get("sha256") != legacy._sha256(model_path):
            raise ValueError(f"successor models[{index}] hash differs")
    ...
    if not (
        ...
        raise ValueError("successor master seed registry differs from matrix")
    ...
    if not (
        int(execution.get("shard_count", -1)) == 2
        and execution.get("devices") == ["cuda:0", "cuda:0"]
        and int(execution.get("parallel_envs_per_shard", -1))
        == int(parallel_envs_per_shard)
    ):
        raise ValueError("successor execution topology differs from master")
    preregistration["execution"]["devices"] = list(execution["devices"])
    binding = {
        "builder": {
    ...
    manifest["successor_contract"] = binding
    preregistration["successor_contract"] = binding
    ...
    if (
        ...
        is not True
        or preregistration["execution"].get("devices")
        != ["cuda:0", "cuda:0"]
    ):
        raise RuntimeError("materialized successor matrix differs from binding")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--stage", choices=tuple(EXPECTED_ATTEMPTS), required=True
    )
    ...
    for path in (manifest_path, preregistration_path):
        if path.exists():
            raise FileExistsError(
                f"refusing to overwrite successor contract: {path}"
            )
