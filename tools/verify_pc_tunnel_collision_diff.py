"""Recompute and freeze a source-bound retail tunnel-collision diff."""
...
from zuma_rl.pc_tunnel_collision import compare_tunnel_collision_transition
...
IMPLEMENTATION_PATHS = (
    ...
    "src/zuma_rl/pc_tunnel_collision.py",
    "tools/verify_pc_tunnel_collision_diff.py",
)
...
def build_report(args: argparse.Namespace) -> dict[str, Any]:
    ...
    source = verify_pc_source_manifest(
        source_manifest,
        evidence_root=root,
        original_root=original_root,
    )
    if source.get("status") != "PASS":
        raise ValueError("source manifest did not pass current verification")
    ...
    report = compare_tunnel_collision_transition(
        frames,
        original_root=original_root,
        level_id=args.level,
        hard=args.hard,
