"""Freeze fresh validation for the recovered reset-aim clone."""
...
PRIOR_PLANS = (
    ...
    SCRIPT_PATH.parents[1]
    / (
        "diagnostics/alphazuma-55-settled-v3-reset-aim-validation-"
        "s99081522-plan-v1.json"
    ),
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if (
        distillation.get("run", {}).get("aim_head_reset", {}).get("enabled")
        is not True
        or distillation.get("startup_recovery", {}).get(
            "failed_before_run_initialization"
        )
        is not True
    ):
        raise ValueError("distillation is not the recovered reset-aim route")
    value = base.build(
    ...
    for prior_path in PRIOR_PLANS:
        ...
        if base._overlap(
            ...
            raise ValueError("recovery validation overlaps an earlier V3 matrix")
    ...
    value["purpose"] = (
        "Paired fresh-seed checkpoint comparison for the recovered reset-aim "
        "clone; the failed route's validation range remains reserved."
    )
    value["models"]["target"]["expected_checkpoints"] = [
        {
            "id": f"settled-v3-reset-aim-v2-epoch-{epoch:02d}",
            "epoch": epoch,
            "expected_path": str(
        ...
        json.dumps(
            {
                ...
                "reserved_failed_validation_range": [1550000400, 1550000454],
                "target_absent": True,
