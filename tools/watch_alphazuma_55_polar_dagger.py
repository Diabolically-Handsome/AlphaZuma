"""Wait for the source polar model, then run a frozen full55 DAgger plan."""
...
from tools.materialize_alphazuma_55_polar_dagger import (
    materialize,
...
def run(
    *,
    ...
    if (
        ...
        raise ValueError("plan binds another polar DAgger watcher")
    status_root = Path(str(plan["outputs"]["status_root"])).resolve()
    if status_root.exists():
        raise FileExistsError(f"polar DAgger watcher root exists: {status_root}")
    ...
    def status(stage: str, **extra: Any) -> None:
        _write_atomic(
            status_path,
            {
                "schema": "zuma-rl.alphazuma-55-polar-dagger-watcher",
                "version": 1,
                "status": "RUNNING",
                "stage": stage,
    ...
    try:
        while not completion_path.exists():
            if source_failure_path.exists():
                raise RuntimeError(
                    "source polar training failed: "
                    + json.dumps(_read(source_failure_path), ensure_ascii=False)
                )
            if not _pid_alive(source_trainer_pid):
                raise RuntimeError(
                    "source polar trainer exited without a terminal receipt"
                )
            status(
                "WAITING_FOR_SOURCE",
                source_trainer_pid=source_trainer_pid,
            )
        ...
        with (status_root / "trainer.stdout.log").open(
            ...
            if process.returncode != 0:
                ...
                raise RuntimeError(
                    f"polar DAgger trainer exited with code {process.returncode}: "
                    + json.dumps(detail, ensure_ascii=False)
                )
        completion = run_dir / "completion.json"
        if not completion.exists():
            raise RuntimeError("polar DAgger trainer omitted completion receipt")
        result = _read(completion)
        if (
            result.get("status") != "COMPLETE"
            or result.get("formal_seed_consumption") is not False
            or result.get("formal_candidate_authority") is not False
        ):
            raise RuntimeError("polar DAgger completion is not eligible")
        final = {
            "schema": "zuma-rl.alphazuma-55-polar-dagger-watcher",
            "version": 1,
            "status": "COMPLETE",
        ...
        failure = {
            "schema": "zuma-rl.alphazuma-55-polar-dagger-watcher-failure",
            "version": 1,
            "status": "ERROR",
            "stage": "ERROR",
            "failed_utc": _utc_now(),
