"""Bind frozen intent-wide checkpoints into paired full55 validation."""
...
def validate_plan_static(plan_path: Path, expected_sha256: str) -> dict[str, Any]:
    ...
    _require(
        ...
        == EXPECTED_EPOCHS,
        "plan does not bind the frozen intent-wide training contract",
    )
    ...
    _require(
        ...
        "invalid intent-wide validation matrix",
    )
...
def materialize(plan_path: Path, expected_sha256: str) -> tuple[Path, Path]:
    ...
    _require(
        ...
        and completion.get("policy_architecture") == "entity_polar_intent_wide"
        and completion.get("training_label_semantics") == "raw_teacher_intent"
        and completion.get("environment_execution_semantics")
        ...
        "intent-wide completion is invalid",
    )
    checkpoint_by_epoch = {
    ...
    _require(
        ...
        "intent-wide completion misses a frozen checkpoint",
    )
    ...
    for epoch, expected_path in zip(EXPECTED_EPOCHS, expected_paths, strict=True):
        ...
        _require(path == expected_path, f"intent-wide epoch {epoch} path differs")
        digest = contracts._sha256(path)
        _require(row["sha256"] == digest, f"intent-wide epoch {epoch} hash mismatch")
        models.append(
            {
                "id": f"polar-intent-wide-epoch-{epoch:02d}",
    ...
    manifest["polar_intent_wide_validation_plan"] = plan_reference
    manifest["target_completion"] = completion_reference
    preregistration["polar_intent_wide_validation_plan"] = plan_reference
