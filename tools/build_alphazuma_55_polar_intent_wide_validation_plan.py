"""Freeze paired full55 validation for the isolated intent-wide policy."""
...
MATERIALIZER = SCRIPT_PATH.with_name(
    "materialize_alphazuma_55_polar_intent_wide_validation.py"
)
WATCHER = SCRIPT_PATH.with_name(
    "watch_alphazuma_55_polar_intent_wide_validation.py"
)
...
def build(
    *,
    ...
    _require(
        ...
        "unexpected intent-wide training preregistration",
    )
    _require(
        training["master_preregistration"]["sha256"] == _sha256(master_path),
        "intent-wide training binds another master",
    )
    ...
    _require(
        ...
        "intent-wide frozen validation changed",
    )
    _require(
        ...
        is False,
        "intent-wide training claims frozen-campaign authority",
    )
    run_dir = Path(str(training["run"]["run_dir"])).resolve()
    _require(
        not (run_dir / "completion.json").exists(),
        "validation plan must be frozen before intent-wide completion",
    )
    ...
    _require(
        ...
        "unexpected intent-wide launch plan",
    )
    _require(
        launch["intent_preregistration"]["sha256"]
        ...
        "intent-wide launch plan does not bind this training run",
    )
    ...
    for path in (
        ...
        _require(not path.exists(), f"intent-wide validation output exists: {path}")
    ...
    return {
        ...
        "source_baseline": {
            ...
            "training_steps": int(source_completion["final_model"]["num_timesteps"]),
        ...
        "wait_for_engineering_slot": {
            "prior_wide_audit": str(prior_wide_audit_path.resolve()),
        ...
        "authority_boundary": {
            "classification": "isolated_intent_label_engineering_validation_only",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--prior-wide-audit", required=True, type=Path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if output.exists():
        raise FileExistsError(
            f"refusing to overwrite intent-wide validation plan: {output}"
        )
    value = build(
