"""Bind frozen raw-intent DAgger rounds into paired full55 validation."""
...
from tools import materialize_alphazuma_55_polar_intent_dagger as training_materializer
...
def validate_plan_static(plan_path: Path, expected_sha256: str) -> dict[str, Any]:
    ...
    _require(
        plan.get("schema")
        == "zuma-rl.alphazuma-55-polar-intent-dagger-validation-plan"
        and plan.get("version") == 1
        and plan.get("status") == "FROZEN_BEFORE_TARGET_PREREGISTRATION",
        "unexpected raw-intent DAgger validation plan",
    )
    ...
    for name, reference in plan["implementation"].items():
        ...
        if name == "materializer":
            _require(
                ...
                "plan binds another validation materializer",
            )
    matrix = plan["matrix"]
    _require(
        ...
        "invalid raw-intent DAgger validation matrix",
    )
    ...
    _require(
        ...
        and boundary["s99081545_successor_candidate_authority"] is False
        and boundary["power_restore_authority"] is False,
        "validation plan claims frozen-campaign authority",
    )
    return plan
...
def materialize(plan_path: Path, expected_sha256: str) -> tuple[Path, Path]:
    ...
    _require(
        ...
        "training preregistration binds another training plan",
    )
    ...
    _require(
        ...
        "raw-intent DAgger completion is invalid",
    )
    ...
    _require(
        ...
        "raw-intent source model changed before validation",
    )
    models = [
        {
            "id": "polar-intent-wide-source-final",
            "training_steps": int(
    ...
    for round_index, expected_path in zip(
        ...
        _require(round_index in round_by_index, "completion misses a DAgger round")
        ...
        models.append(
            {
                "id": f"polar-intent-dagger-round-{round_index:02d}",
    ...
    manifest["polar_intent_dagger_validation_plan"] = plan_reference
    manifest["training_preregistration"] = training_reference
    manifest["target_completion"] = completion_reference
    preregistration["polar_intent_dagger_validation_plan"] = plan_reference
    preregistration["training_preregistration"] = training_reference
