"""Freeze paired full55 validation before raw-intent DAgger starts."""
...
MATERIALIZER = SCRIPT_PATH.with_name(
    "materialize_alphazuma_55_polar_intent_dagger_validation.py"
)
WATCHER = SCRIPT_PATH.with_name(
    "watch_alphazuma_55_polar_intent_dagger_validation.py"
)
...
def build(*, project_root: Path) -> dict[str, Any]:
    ...
    _require(
        ...
        "raw-intent DAgger frozen validation changed",
    )
    ...
    output_root = Path(
        ...
        "validation-s99081550-v1"
    )
    manifest_output = (
        root
        / "diagnostics/alphazuma-55-polar-intent-dagger-validation-"
        "s99081550-models-v1.json"
    )
    evaluation_output = (
        root
        / "diagnostics/alphazuma-55-polar-intent-dagger-validation-"
        "s99081550-preregistration-v1.json"
    )
    audit_output = (
        root
        / "diagnostics/alphazuma-55-polar-intent-dagger-validation-"
        "s99081550-audit-v1.json"
    )
    status_root = Path(
        "/mnt/d/ZumaTraining/alphazuma-55-polar-intent-dagger-"
        "validation-s99081550-watch-v1"
    )
    for path in (
        output_root,
        manifest_output,
