"""Run frozen autonomy validation after training and the prior matrix."""
...
from tools import materialize_alphazuma_55_polar_intent_autonomy_validation as binding
...
def run(
    *, plan_path: Path, expected_plan_sha256: str, poll_seconds: float
    ...
    if (
        ...
        raise ValueError("plan binds another autonomy validation watcher")
    status_root = Path(plan["outputs"]["status_root"]).resolve()
    if status_root.exists():
        raise FileExistsError(
            f"autonomy validation status root exists: {status_root}"
        )
    ...
    def status(stage: str, **extra: Any) -> None:
        _write_atomic(
            ...
            {
                "schema": (
                    "zuma-rl.alphazuma-55-polar-intent-autonomy-"
                    "validation-watcher"
                ),
                "version": 1,
                "status": "RUNNING",
    ...
    expected_attempts = int(plan["matrix"]["expected_attempts"])
    try:
        while True:
            if target_failure_path.exists():
                raise RuntimeError(
                    "autonomy training failed: "
            ...
            if launch_status_path.exists():
                ...
                if terminal == "COMPLETE":
                    if not completion_path.exists():
                        raise RuntimeError(
                            "autonomy controller completed without receipt"
                        )
                    break
                if terminal in {"FAILED", "ERROR"}:
                    raise RuntimeError(
                        "autonomy launch controller failed: "
        ...
        if prior_audit.get("status") != "PASS":
            raise RuntimeError("prior engineering audit did not PASS")

        status("MATERIALIZING_VALIDATION")
        manifest_path, preregistration_path = binding.materialize(
        ...
        with (status_root / "evaluator.stdout.log").open(
            ...
            if process.returncode != 0:
                raise RuntimeError(
                    f"autonomy evaluator exited with code {process.returncode}"
                )
        matrix = _matrix_snapshot(matrix_path)
        if (
            matrix is None
            or matrix["status"] != "COMPLETE"
            or int(matrix["completed_attempts"]) != expected_attempts
            or int(matrix["expected_attempts"]) != expected_attempts
        ):
            raise RuntimeError("autonomy validation matrix is not complete")
        ...
        if result.returncode != 0:
            raise RuntimeError(
                f"autonomy auditor exited with code {result.returncode}"
            )
        audit = _read(audit_output)
        if audit.get("status") != "PASS":
            raise RuntimeError("autonomy independent audit did not PASS")
        final = {
            "schema": (
                "zuma-rl.alphazuma-55-polar-intent-autonomy-"
                "validation-watcher"
