"""Paired evaluator for base and actor-visible motor observations.

Both profiles use identical game, reward, and elite-human actuator dynamics.
Only the actor observation differs: legacy models receive the frozen base
observation, while motor-observable models receive the same exact prefix plus
the deterministic actuator state that affects future executed actions.
"""
...
from tools.evaluate_zero_shot_multilevel_v2 import (
    CapacityAwareEpisodeTracker,
    ObservationCapacityFailClosed,
)
...
BASE_OBSERVATION_PROFILE = "human-speedrun-v1"
SUPPORTED_OBSERVATION_PROFILES = frozenset(
    {BASE_OBSERVATION_PROFILE, MOTOR_OBSERVATION_PROFILE}
)
EpisodeTracker = CapacityAwareEpisodeTracker


def _observation_profile(model_spec: dict[str, Any]) -> str:
    profile = str(model_spec.get("observation_profile", ""))
    if profile not in SUPPORTED_OBSERVATION_PROFILES:
        raise ValueError(
            f"unsupported or missing observation profile: {profile!r}"
        )
    return profile
...
def _make_env_factory(
    *,
    ...
    if profile not in SUPPORTED_OBSERVATION_PROFILES:
        raise ValueError(f"unsupported observation profile: {profile!r}")
    ...
    def make_env() -> HumanSpeedrunWrapper:
        ...
        wrapper = (
            ObservableHumanSpeedrunWrapper
            if profile == MOTOR_OBSERVATION_PROFILE
            else HumanSpeedrunWrapper
        )
        return wrapper(
            guarded,
...
def _validate_model_spaces(
    *,
    ...
    for model_spec in specs:
        observation_profile = _observation_profile(model_spec)
