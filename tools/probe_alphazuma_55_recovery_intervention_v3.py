"""Probe semantic-override deep recovery for AlphaZuma full55."""
...
CONTROLLER_PATH = (
    SCRIPT_PATH.parent / "alphazuma_55_recovery_intervention_v3.py"
).resolve()
...
def _validate_preregistration(path: Path) -> dict[str, Any]:
    ...
    if not (
        prereg.get("schema")
        == "zuma-rl.alphazuma-55-recovery-intervention-v3-probe-preregistration"
        and prereg.get("version") == 1
        and prereg.get("status") == "FROZEN_BEFORE_PROBE"
    ):
        raise ValueError("unexpected recovery intervention V3 preregistration")
    ...
    evidence = prereg.get("v2_failure_evidence", {})
    if not isinstance(evidence, dict):
        raise ValueError("V2 failure evidence must be an object")
    evidence_path = Path(str(evidence.get("path", ""))).resolve(strict=True)
    if evidence.get("sha256") != base._sha256(evidence_path):
        raise ValueError("V2 failure evidence bytes changed")
    ...
    checks = evidence_value.get("decision", {}).get("checks", {})
    expected_checks = {
        "intervention_wins_exceed_student_only": True,
        "intervention_total_score_exceeds_student_only": True,
        "paired_score_improvements": True,
        "minimum_interventions": True,
        "minimum_recovery_successes": True,
        "maximum_teacher_execution_fraction": False,
    }
    if not (
        evidence_value.get("schema")
        == "zuma-rl.alphazuma-55-recovery-intervention-v2-probe-completion"
        and evidence_value.get("status") == "COMPLETE"
        and evidence_value.get("decision", {}).get("status") == "FAIL"
        and checks == expected_checks
        and int(v2_recovery.get("summary", {}).get("wins", -1)) == 8
        and float(
            v2_recovery.get("summary", {}).get(
                "teacher_execution_fraction", 0.0
            )
        )
        > 0.6
    ):
        raise ValueError("V2 failure evidence no longer matches the correction")
    ...
    if not (
        ...
        and probe.get("teacher_executes_only_on_semantic_disagreement") is True
    ):
        raise ValueError("paired V3 execution contract changed")
    frozen_controller = RecoveryInterventionConfigV3(
        **dict(probe.get("controller_config", {}))
    )
    if frozen_controller.contract() != probe.get("controller_config"):
        raise ValueError("V3 controller configuration is not canonical")
    ...
    if prereg.get("acceptance") != expected_acceptance:
        raise ValueError("V3 probe acceptance contract changed")
    ...
    for name in (
        ...
        if boundary.get(name) is not False:
            raise ValueError(f"V3 probe authority changed: {name}")
    return prereg
...
def run(*, preregistration_path: Path, original_root: Path) -> dict[str, Any]:
    ...
    if run_dir.exists():
        raise FileExistsError(f"V3 probe output already exists: {run_dir}")
    run_dir.mkdir(parents=True)
    started = time.perf_counter()
    base._write_atomic(
        run_dir / "config.json",
        {
            "schema": "zuma-rl.alphazuma-55-recovery-intervention-v3-probe-config",
            "version": 1,
            "status": "FROZEN",
            "preregistration": {
    ...
    try:
        ...
        controller_config = RecoveryInterventionConfigV3(
            **dict(prereg["paired_probe"]["controller_config"])
        )
        base.RecoveryInterventionController = RecoveryInterventionControllerV3
        modes = []
        for mode in EXPECTED_MODES:
            base._write_atomic(
                ...
                {
                    "schema": (
                        "zuma-rl.alphazuma-55-recovery-intervention-v3-probe-status"
                    ),
                    "version": 1,
                    "status": "RUNNING",
                    "stage": mode,
        ...
        completion = {
            "schema": "zuma-rl.alphazuma-55-recovery-intervention-v3-probe-completion",
            "version": 1,
            "status": "COMPLETE",
            ...
            "v2_failure_evidence": prereg["v2_failure_evidence"],
            "modes": modes,
            "decision": decision,
        ...
        base._write_atomic(
            ...
            {
                "schema": "zuma-rl.alphazuma-55-recovery-intervention-v3-probe-failure",
                "version": 1,
                "status": "FAILED",
