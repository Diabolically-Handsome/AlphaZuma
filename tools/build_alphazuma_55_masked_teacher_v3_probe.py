"""Freeze a fresh exact-action-mask probe for teacher V3."""
...
EVALUATOR = PROJECT_ROOT / "tools/evaluate_alphazuma_55_masked_teacher_v3.py"
...
V3_FRESH_RESULT = Path(
    "/mnt/d/ZumaTraining/alphazuma-55-weekend-s81081401-v1/engineering/"
    "settled-teacher-v3-fresh-probe-s99081514-v1/result.json"
)
SEED_FIRST = 1_400_960_000
...
def build(*, output_root: Path, workers: int) -> dict[str, Any]:
    ...
    if not (
        ...
        raise ValueError("masked V3 seeds escaped engineering namespace")
    environment = copy.deepcopy(source["environment"])
    environment["base_config"]["max_ticks"] = 30_000
    return {
        "schema": "zuma-rl.alphazuma-55-masked-teacher-v3-probe-preregistration",
        "version": 1,
        "status": "FROZEN_BEFORE_FRESH_ENGINEERING_PROBE",
        "created_utc": datetime.now(timezone.utc).isoformat(),
        "campaign_id": master["campaign_id"],
        "objective": (
            "Measure V3 after replacing every temporarily masked button request "
            "with wait while preserving its selected aim."
        ),
        ...
        "teacher": {
            "policy_id": POLICY_ID,
            "base_policy_id": "curve-aware-settled-strategic-actor-observable-v3",
        ...
        "development_disclosure": {
            ...
            "v3_unmasked_fresh_result": legacy._artifact(V3_FRESH_RESULT),
            "fresh_seed_relative_to_mask_adapter_bytes": True,
        ...
        "execution": {
            ...
            "max_ticks": 30_000,
            "action_masks": "exact_get_action_masks_before_every_decision",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if output_root.exists():
        raise FileExistsError(f"masked V3 output root exists: {output_root}")
