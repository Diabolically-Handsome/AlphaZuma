"""Freeze the fresh long-horizon probe for settled teacher V2."""
...
EVALUATOR = PROJECT_ROOT / "tools/evaluate_alphazuma_55_settled_teacher_v2.py"
SETTLED_V2 = PROJECT_ROOT / "src/zuma_rl/revenge_settled_strategic_teacher_v2.py"
...
V1_FRESH_RESULT = Path(
    "/mnt/d/ZumaTraining/alphazuma-55-weekend-s81081401-v1/engineering/"
    "settled-teacher-fresh-probe-s99081512-v1/result.json"
)
SEED_FIRST = 1_400_920_000
...
def build(*, output_root: Path, workers: int) -> dict[str, Any]:
    ...
    if not (
        ...
        raise ValueError("V2 probe seeds escaped engineering namespace")
    environment = copy.deepcopy(source["environment"])
    environment["base_config"]["max_ticks"] = 30_000
    return {
        "schema": "zuma-rl.alphazuma-55-settled-teacher-v2-probe-preregistration",
        "version": 1,
        "status": "FROZEN_BEFORE_FRESH_ENGINEERING_PROBE",
        "created_utc": datetime.now(timezone.utc).isoformat(),
        "campaign_id": master["campaign_id"],
        "objective": (
            "Measure fresh 55-level coverage after preparing both fire and swap "
            "targets, with a 300-second engineering horizon."
        ),
        ...
        "development_disclosure": {
            ...
            "v1_fresh_result": legacy._artifact(V1_FRESH_RESULT),
            "v2_calibration_seeds_already_consumed": [
                1400700100,
                1400900009,
                1400900046,
            ],
            "long_horizon_calibration_reused_v1_probe_seeds": True,
            "fresh_seed_relative_to_frozen_v2_teacher_bytes": True,
        ...
        "implementation": {
            ...
            "settled_teacher_v2": legacy._artifact(SETTLED_V2),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if output_root.exists():
        raise FileExistsError(f"settled teacher V2 output root exists: {output_root}")
