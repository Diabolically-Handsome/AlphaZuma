"""Extend the formal seed registry for the all-actions motor successor."""
...
CAMPAIGN_ID = "motor-observable-gradual-allaim-successor-s99081636"
...
def _write_new(path: Path, value: dict[str, Any]) -> None:
    ...
    if resolved.exists():
        temporary.unlink()
        raise FileExistsError(f"formal seed registry already exists: {resolved}")
...
def build(*, expected_master_sha256: str, output: Path) -> dict[str, Any]:
    ...
    master = _read(MASTER)
    final = master.get("seed_registry", {}).get("final_blind", {})
    continuous = master.get("seed_registry", {}).get(
        "continuous_campaign_challenge", {}
    )
    if not (
        master.get("campaign_id")
        == "alphazuma-55-motor-observable-gradual-allaim-successor-"
        "s99081636-v1"
        and Path(
            str(
        ...
        == output.resolve()
        and int(final.get("first", -1)) == 3_700_000_000
        and int(final.get("last", -1)) == 3_700_000_439
        and int(continuous.get("first", -1)) == 3_800_000_000
        and int(continuous.get("last", -1)) == 3_800_000_219
    ):
        raise ValueError("all-action successor master seed contract differs")
    campaigns = list(predecessor.get("campaigns", []))
    if any(str(row.get("id")) == CAMPAIGN_ID for row in campaigns):
        raise ValueError("all-action successor already appears in predecessor")
    campaigns.append(
        {
            "id": CAMPAIGN_ID,
            "master": master_ref,
