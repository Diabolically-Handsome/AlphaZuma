"""Evaluate the frozen V2 settled teacher on a fresh 55-level matrix."""
...
from tools import evaluate_alphazuma_55_settled_teacher as legacy
...
POLICY_ID = "aim-settled-strategic-actor-observable-v2"
...
def _validate_preregistration(path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if (
        prereg.get("schema")
        != "zuma-rl.alphazuma-55-settled-teacher-v2-probe-preregistration"
        or prereg.get("version") != 1
        or prereg.get("status") != "FROZEN_BEFORE_FRESH_ENGINEERING_PROBE"
    ):
        raise ValueError("unexpected settled-teacher-v2 preregistration")
    evaluator = geometric._bound_path(
        prereg["implementation"]["evaluator"], "evaluator"
    )
    if evaluator != SCRIPT_PATH:
        raise ValueError("preregistration binds another V2 evaluator")
    for key, name in (
        ("settled_teacher_v2", "settled teacher V2"),
    ...
    if not isinstance(levels, list) or len(levels) != 55:
        raise ValueError("V2 probe must contain exactly 55 levels")
    if not isinstance(tasks, list) or len(tasks) != 55:
        raise ValueError("V2 probe must contain exactly 55 tasks")
    ids = [str(level["id"]) for level in levels]
    if ids != [str(level["id"]) for level in source["levels"]]:
        raise ValueError("V2 probe level inventory differs from source")
    if len(set(ids)) != 55 or [str(task["level_id"]) for task in tasks] != ids:
        raise ValueError("V2 probe task matrix is invalid")
    if any(str(task["teacher_policy_id"]) != POLICY_ID for task in tasks):
        raise ValueError("V2 probe task references another policy")
    seeds = [int(task["seed"]) for task in tasks]
    seed_range = prereg["seed_range"]
    if seeds != list(range(int(seed_range[0]), int(seed_range[1]) + 1)):
        raise ValueError("V2 probe seeds are not contiguous")
    if not (1_400_000_000 <= seeds[0] <= seeds[-1] <= 1_400_999_999):
        raise ValueError("V2 probe escaped the engineering namespace")
    if int(prereg["teacher"]["aim_tolerance_bins"]) != 2:
        raise ValueError("V2 tolerance differs from frozen teacher")
    environment = prereg.get("environment")
    source_environment = source.get("environment")
    if not isinstance(environment, dict) or not isinstance(source_environment, dict):
        raise ValueError("V2 environment contract is missing")
    expected = copy.deepcopy(source_environment)
    expected["base_config"]["max_ticks"] = 30_000
    if environment != expected:
        raise ValueError("V2 environment differs beyond the frozen horizon")
    workers = int(prereg["execution"]["workers"])
    if not 1 <= workers <= 24:
        raise ValueError("V2 workers are outside [1, 24]")
...
def _run_task(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    row = legacy._BASE_RUN_TASK(payload)
