"""Freeze no-inference formal intent for a replay-anchored successor."""
...
FINAL_BASE = 3_100_000_000
FINAL_LAST = FINAL_BASE + 55 * 8 - 1
CONTINUOUS_BASE = 3_200_000_000
...
PROMOTABLE_CANDIDATES = EXPECTED_CANDIDATES[2:]
...
def build(
    *,
    ...
    _require(
        ...
        "replay-anchor engineering matrix differs",
    )
    _require(
    ...
    protected.extend(_ranges(predecessor.get("formal_campaign_if_promoted", {})))
    for reserved in proposed:
        _require(
            all(_disjoint(reserved, prior) for prior in protected),
            "proposed replay-anchor formal seeds overlap prior ranges",
        )
        _require(
            0 <= reserved[0] <= reserved[1] <= 0xFFFFFFFF,
            "proposed replay-anchor formal seeds exceed uint32",
        )
    ...
    return {
        "schema": (
            ...
            "replay-anchor-successor-intent"
        ),
        "version": 1,
        ...
        "objective": (
            ...
            "replay-anchored student round clears the frozen engineering Gate."
        ),
