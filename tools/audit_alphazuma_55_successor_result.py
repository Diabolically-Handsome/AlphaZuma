"""Independently audit an AlphaZuma 55 successor certification result."""
...
def audit(master_path: Path) -> dict[str, Any]:
    ...
    _require(engineering.get("eligible_model_ids") == EXPECTED_IDS, "eligible ids differ")
    ...
    promotion_pass = (
        int(ranking[0]["metrics"]["cleared_levels"]) >= minimum_levels
    ...
    if not promotion_pass:
        ...
        _require(
            ...
            "formal successor output exists despite failed promotion",
        )
        return {
            "schema": "zuma-rl.alphazuma-55-successor-independent-audit",
            "version": 1,
            "status": "PASS",
    ...
    _require(
        ...
        "successor controller is not COMPLETE",
    )
    _require(report.get("status") == "COMPLETE", "successor final report is incomplete")
    ...
    for stage, manifest_file, prereg_file, seed, attempts in (
        ("final_blind", final_manifest_path, final_prereg_path, 2_100_000_000, 8),
        ("continuous", continuous_manifest_path, continuous_prereg_path, 2_200_000_000, 4),
    ):
        ...
        binding = stage_prereg.get("successor_contract")
        _require(isinstance(binding, dict), f"{stage} successor binding is absent")
        _require(
            binding.get("builder")
        ...
        _require(
            ...
            is True,
            f"{stage} evaluator or seed freeze differs",
        )
