"""Freeze the no-inference intent for a possible wide-polar successor."""
...
FINAL_BASE = 2_300_000_000
FINAL_LAST = FINAL_BASE + 55 * 8 - 1
CONTINUOUS_BASE = 2_400_000_000
...
WIDE_CANDIDATES = EXPECTED_CANDIDATES[1:]
...
def _assert_disjoint_from_source(source: dict[str, Any]) -> None:
    proposed = ((FINAL_BASE, FINAL_LAST), (CONTINUOUS_BASE, CONTINUOUS_LAST))
    for name, registry in source.get("seed_registry", {}).items():
        ...
        for new_range in proposed:
            _require(
                new_range[1] < source_range[0]
                or source_range[1] < new_range[0],
...
def build(
    *,
    ...
    _require(len(levels) == 55 and len(set(levels)) == 55, "source scope is not 55 levels")
    ...
    _require(
        ...
        "unexpected wide validation plan",
    )
    _require(
        validation["master_preregistration"]["sha256"]
        == _sha256(source_master_path),
        "wide validation binds another source master",
    )
    ...
    _require(
        ...
        "wide engineering matrix differs",
    )
    _require(
        ...
        is False,
        "wide engineering plan claims formal authority",
    )
    ...
    return {
        ...
        "objective": (
            ...
            "a frozen wide-polar checkpoint beats the source model and clears the "
        ...
        "promotion_gate": {
            "selected_model_must_be_one_of": list(WIDE_CANDIDATES),
        ...
        "outputs": {
            ...
            "independent_audit_receipt": str(
                independent_audit_receipt.resolve()
            ),
        },
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--wide-validation-plan", required=True, type=Path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    value = build(
        ...
        validation_plan_path=args.wide_validation_plan.expanduser(),
