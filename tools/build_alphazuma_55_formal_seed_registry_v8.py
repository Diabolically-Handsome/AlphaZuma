"""Extend the formal seed registry for the V2 all-actions successor."""
...
PREDECESSOR = PROJECT_ROOT / (
    "diagnostics/alphazuma-55-weekend-formal-seed-registry-s99081632-v6.json"
)
PREDECESSOR_SHA256 = (
...
MASTER = PROJECT_ROOT / (
    ...
    "successor-s99081636-preregistration-v2.json"
)
DEFAULT_OUTPUT = PROJECT_ROOT / (
    "diagnostics/alphazuma-55-weekend-formal-seed-registry-s99081641-v8.json"
)
CAMPAIGN_ID = "motor-observable-gradual-allaim-successor-s99081636-v2"
...
def _validate_ranges(campaigns: list[dict[str, Any]]) -> None:
    ...
    for campaign in campaigns:
        for row in campaign.get("ranges", []):
            ...
            rows.append((first, last, str(campaign["id"]), str(row["stage"])))
    rows.sort()
...
def build(*, expected_master_sha256: str, output: Path) -> dict[str, Any]:
    ...
    if not (
        ...
        == "alphazuma-55-motor-observable-gradual-allaim-successor-s99081636-v2"
        and Path(master["formal_seed_registry"]["active_registry_path"]).resolve()
        == output.resolve()
        and [int(final["first"]), int(final["last"])]
        == [3_700_000_000, 3_700_000_439]
        and [int(continuous["first"]), int(continuous["last"])]
        == [3_800_000_000, 3_800_000_219]
    ):
        raise ValueError("V2 all-actions successor seed contract differs")
    campaigns = list(predecessor.get("campaigns", []))
    if any(str(row.get("id")) == CAMPAIGN_ID for row in campaigns):
        raise ValueError("V2 all-actions successor already registered")
    campaigns.append(
        {
            "id": CAMPAIGN_ID,
            "master": master_ref,
