...
def run(*, preregistration_path: Path, original_root: Path) -> dict[str, Any]:
    ...
    def write_status(stage: str, **extra: Any) -> None:
        intent_dagger._write_atomic(
            run_dir / "training_status.json",
            {
                ...
                "status": "RUNNING",
                "stage": stage,
                "updated_utc": intent_dagger._utc_now(),
    ...
    try:
        ...
        intent_dagger._write_atomic(run_dir / "migration.json", migration)
        model = target
        del source
        ...
        completion = {
            ...
            "version": 1,
            "status": "COMPLETE",
            "completed_utc": intent_dagger._utc_now(),
            "wall_seconds": time.perf_counter() - started,
            "policy_architecture": (
                "entity_polar_intent_motor_observable_replay"
            ),
        ...
        intent_dagger._write_atomic(run_dir / "completion.json", completion)
        return completion
        ...
        failure = {
            ...
            "version": 1,
            "status": "ERROR",
            "failed_utc": intent_dagger._utc_now(),
            "wall_seconds": time.perf_counter() - started,
        ...
        intent_dagger._write_atomic(run_dir / "failure.json", failure)
        raise
    finally:
