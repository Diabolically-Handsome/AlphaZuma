"""Bind the frozen raw-intent DAgger final model into autonomy training."""
...
from tools.distill_alphazuma_55_polar_intent_autonomy_v1 import (
...
def validate_plan_static(path: Path, expected_sha256: str) -> dict[str, Any]:
    ...
    if legacy._sha256(path) != expected_sha256:
        raise ValueError("autonomy continuation plan hash differs")
    plan = _read(path)
    if not (
        plan.get("schema")
        == "zuma-rl.alphazuma-55-polar-intent-autonomy-plan"
        and plan.get("version") == 1
        and plan.get("status")
        == "FROZEN_BEFORE_SOURCE_TERMINAL_RESULT_INSPECTION"
    ):
        raise ValueError("unexpected autonomy continuation plan")
    ...
    if not (
        ...
        raise ValueError("unexpected source DAgger preregistration")
    for name, artifact in plan.get("implementation", {}).items():
        ...
        if name == "materializer" and implementation != SCRIPT_PATH:
            raise ValueError("plan binds another autonomy materializer")
    ...
    if not (
        ...
        raise ValueError("autonomy continuation escaped training registry")
    ...
    if not (
        ...
        raise ValueError("autonomy validation seeds changed")
...
def materialize(plan_path: Path, expected_plan_sha256: str) -> Path:
    ...
    if model.get("sha256") != model_sha256:
        raise ValueError("source DAgger model differs from completion receipt")
    preregistration = {
        "schema": (
