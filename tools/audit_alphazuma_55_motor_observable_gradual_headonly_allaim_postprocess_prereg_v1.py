"""Independently audit the factorial postprocess plan before inference."""
...
def audit(*, plan_path: Path, expected_plan_sha256: str) -> dict[str, Any]:
    ...
    if _sha256(plan_path) != expected_plan_sha256:
        raise ValueError("factorial postprocess plan hash differs")
    ...
    training_lineage = plan.get("training_seed_lineage", {})
    if not (
        plan.get("status")
        ...
        and int(screen.get("base_seed", -1)) == 1_550_005_000
        and int(screen.get("last_seed", -1)) == 1_550_005_011
        and int(screen.get("expected_candidates", -1)) == 17
        and int(screen.get("expected_attempts", -1)) == 204
        and int(gate.get("base_seed", -1)) == 1_550_005_100
        and int(gate.get("last_seed", -1)) == 1_550_005_154
        ...
        == "NONE"
        and training_lineage.get("classification")
        == "fresh_engineering_training_subrange"
        and training_lineage.get("range") == [1_546_001_000, 1_546_001_219]
        and training_lineage.get("formal_seed_overlap") is False
        ...
        raise ValueError("factorial postprocess preregistration differs")

    return {
        "schema": "zuma-rl.alphazuma-55-headonly-allaim-postprocess-prereg-audit",
        "version": 1,
        "status": "PASS",
        ...
        "matrix": {
            ...
            "screen_attempts": 204,
            "screen_seed_range": [1_550_005_000, 1_550_005_011],
            "maximum_gate_candidates": 5,
            "maximum_gate_attempts": 275,
            "gate_seed_range": [1_550_005_100, 1_550_005_154],
            "minimum_wins": 35,
            "minimum_cleared_levels": 35,
        },
        "factorial_contract": {
            "variant": "action_head_only_all_actions_aim_v1",
