"""Run frozen wide-polar validation after training and the prior matrix."""
...
from tools.materialize_alphazuma_55_polar_wide_validation import (
    materialize,
...
def run(
    *, plan_path: Path, expected_plan_sha256: str, poll_seconds: float
    ...
    if status_root.exists():
        raise FileExistsError(f"wide validation status root exists: {status_root}")
    ...
    def status(stage: str, **extra: Any) -> None:
        _write_atomic(
            status_path,
            {
                "schema": "zuma-rl.alphazuma-55-polar-wide-validation-watcher",
                "version": 1,
                "status": "RUNNING",
                "stage": stage,
    ...
    try:
        while not completion_path.exists():
            ...
            if trainer_pid is not None and not _pid_alive(trainer_pid):
                raise RuntimeError(
                    "wide trainer exited without completion or failure receipt"
                )
            status(
        ...
        if completion.get("status") != "COMPLETE":
            raise ValueError("wide completion status is not COMPLETE")
        prior_audit_path = Path(
            plan["wait_for_engineering_slot"]["prior_dagger_audit"]
        )
        ...
        if prior_audit.get("status") != "PASS":
            raise RuntimeError("prior DAgger engineering audit did not PASS")
        ...
        with (status_root / "evaluator.stdout.log").open(
            ...
            if process.returncode != 0:
                raise RuntimeError(
                    f"wide evaluator exited with code {process.returncode}"
                )
        matrix = _matrix_snapshot(matrix_path)
        if (
            matrix is None
            ...
            raise RuntimeError("wide validation matrix is not complete")
        ...
        if result.returncode != 0:
            raise RuntimeError(f"wide auditor exited with code {result.returncode}")
        audit = _read(audit_output)
        if audit.get("status") != "PASS":
            raise RuntimeError("wide independent audit did not PASS")
        final = {
            "schema": "zuma-rl.alphazuma-55-polar-wide-validation-watcher",
            "version": 1,
            "status": "COMPLETE",
        ...
        failure = {
            "schema": "zuma-rl.alphazuma-55-polar-wide-validation-watcher-failure",
            "version": 1,
            "status": "ERROR",
            "failed_utc": _utc_now(),
            "wall_seconds": time.perf_counter() - started,
