"""Bind V3 high-fidelity checkpoints into their frozen fresh validation."""
...
def validate_plan_static(plan_path: Path, expected_sha256: str) -> dict[str, Any]:
    _require(
        ...
        "high-fidelity validation plan hash mismatch",
    )
    ...
    _require(
        ...
        "unexpected high-fidelity validation plan",
    )
    ...
    _require(
        ...
        "invalid high-fidelity validation matrix",
    )
    _require(
        tuple(
            int(row["epoch"])
            for row in plan["models"]["target"]["expected_checkpoints"]
        )
        == EXPECTED_EPOCHS,
        "high-fidelity checkpoint schedule differs",
    )
    _require(
...
def materialize(plan_path: Path, expected_sha256: str) -> tuple[Path, Path]:
    ...
    _require(
        ...
        "high-fidelity distillation is not complete",
    )
    checkpoint_by_epoch = {
        int(row["epoch"]): row
    ...
    manifest["settled_v3_high_fidelity_validation_plan"] = plan_reference
    manifest["target_completion"] = completion_reference
    preregistration["settled_v3_high_fidelity_validation_plan"] = plan_reference
