"""Bind frozen autonomy rounds into paired full55 engineering validation."""
...
from tools import materialize_alphazuma_55_polar_intent_autonomy as training_materializer
...
def validate_plan_static(plan_path: Path, expected_sha256: str) -> dict[str, Any]:
    ...
    _require(
        ...
        "unexpected autonomy validation plan",
    )
    ...
    for name, reference in plan["implementation"].items():
        ...
        if name == "materializer":
            _require(
                ...
                "plan binds another autonomy validation materializer",
            )
    matrix = plan["matrix"]
    _require(
        ...
        "invalid autonomy validation matrix",
    )
    ...
    _require(
        ...
        and boundary["predecessor_successor_candidate_authority"] is False
        and boundary["power_restore_authority"] is False,
        "autonomy validation claims formal authority",
    )
    return plan
...
def materialize(plan_path: Path, expected_sha256: str) -> tuple[Path, Path]:
    ...
    _require(
        ...
        "training preregistration binds another autonomy plan",
    )
    ...
    _require(
        ...
        "autonomy completion is invalid",
    )
    ...
    _require(
        ...
        "autonomy source model changed before validation",
    )
    models = [
        {
            "id": "polar-intent-autonomy-source-final",
            "training_steps": int(
    ...
    for round_index, expected_path in zip(
        ...
        _require(
            round_index in round_by_index,
            "completion misses an autonomy round",
        )
        ...
        models.append(
            {
                "id": f"polar-intent-autonomy-round-{round_index:02d}",
    ...
    manifest["polar_intent_autonomy_validation_plan"] = plan_reference
    manifest["training_preregistration"] = training_reference
    manifest["target_completion"] = completion_reference
    preregistration["polar_intent_autonomy_validation_plan"] = plan_reference
    ...
    _require(
        ...
        "materialized seed matrix differs from autonomy plan",
    )
