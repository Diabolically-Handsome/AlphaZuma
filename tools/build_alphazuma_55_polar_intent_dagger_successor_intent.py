"""Freeze no-inference formal intent for a raw-intent DAgger successor."""
...
from tools.materialize_alphazuma_55_polar_intent_dagger_validation import (
    validate_plan_static,
)


SCRIPT_PATH = Path(__file__).resolve()
FINAL_BASE = 2_700_000_000
FINAL_LAST = FINAL_BASE + 55 * 8 - 1
CONTINUOUS_BASE = 2_800_000_000
CONTINUOUS_LAST = CONTINUOUS_BASE + 55 * 4 - 1
EXPECTED_CANDIDATES = (
    "polar-intent-wide-source-final",
...
def build(
    *,
    ...
    sibling_intent_path = sibling_intent_path.resolve(strict=True)
    ...
    protected.extend(_ranges(sibling.get("formal_campaign_if_promoted", {})))
    for reserved in proposed:
        _require(
            all(_disjoint(reserved, prior) for prior in protected),
            "proposed raw-intent DAgger formal seeds overlap prior ranges",
        )
        _require(
            0 <= reserved[0] <= reserved[1] <= 0xFFFFFFFF,
            "proposed formal seeds exceed uint32",
        )
    ...
    return {
        ...
        "objective": (
            "Authorize formal full55 and continuous evaluation only if a "
            "raw-intent DAgger round clears its predeclared engineering gate."
        ),
        "source_campaign_master": _artifact(source_master_path),
        "formal_seed_registry_snapshot": _artifact(formal_registry_path),
        "sibling_reserved_seed_intent": _artifact(sibling_intent_path),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--sibling-intent", required=True, type=Path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    value = build(
        ...
        sibling_intent_path=args.sibling_intent.expanduser(),
