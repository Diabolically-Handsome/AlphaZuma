"""Freeze paired validation for the isolated full55 polar DAgger policy."""
...
MATERIALIZER = SCRIPT_PATH.with_name(
    "materialize_alphazuma_55_polar_dagger_validation.py"
)
WATCHER = SCRIPT_PATH.with_name(
    "watch_alphazuma_55_polar_dagger_validation.py"
)
...
def build(
    *,
    ...
    dagger_plan_path: Path,
    original_root: Path,
    output_root: Path,
    manifest_output: Path,
    evaluation_preregistration_output: Path,
    audit_output: Path,
    prior_polar_audit_path: Path,
    status_root: Path,
    device: str,
    parallel_envs: int,
) -> dict[str, Any]:
    ...
    _require(
        ...
        "unexpected polar DAgger training preregistration",
    )
    _require(
        training["master_preregistration"]["sha256"] == _sha256(master_path),
        "polar DAgger training binds another master",
    )
    dagger_plan = _read_json(dagger_plan_path)
    _require(
        dagger_plan.get("schema") == "zuma-rl.alphazuma-55-polar-dagger-plan"
        and training["materialization_plan"]["sha256"]
        == _sha256(dagger_plan_path),
        "training does not bind the supplied DAgger plan",
    )
    ...
    _require(
        ...
        "polar DAgger frozen validation changed",
    )
    _require(
        training["authority_boundary"]["current_campaign_candidate_authority"]
        is False,
        "polar DAgger training claims current-campaign authority",
    )
    ...
    for path in (
        ...
        _require(not path.exists(), f"DAgger validation output exists: {path}")
    ...
    return {
        ...
        "dagger_plan": _artifact(dagger_plan_path),
        "target": {
            "run_dir": str(run_dir),
            ...
            "round_indices": [0, 1, 2],
            "round_model_paths": [
                str(run_dir / f"round_{index:02d}/final_model.zip")
                for index in (0, 1, 2)
            ],
        },
        "matrix": {
        ...
        "wait_for_engineering_slot": {
            "prior_polar_audit": str(prior_polar_audit_path.resolve()),
            "required_status": "PASS",
            "reason": "serialize the two 24-env engineering matrices",
        },
        "outputs": {
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--dagger-plan", required=True, type=Path)
    ...
    parser.add_argument("--prior-polar-audit", required=True, type=Path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    value = build(
        ...
        dagger_plan_path=args.dagger_plan.expanduser().resolve(strict=True),
        ...
        prior_polar_audit_path=args.prior_polar_audit.expanduser(),
