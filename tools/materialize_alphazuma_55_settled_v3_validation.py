"""Bind completed settled-V3 checkpoints into their frozen validation plan."""
...
def validate_plan_static(
    ...
    matrix = plan["matrix"]
    _require(int(matrix["levels"]) == 55, "V3 validation must contain 55 levels")
    _require(
        int(matrix["last_seed"]) == int(matrix["base_seed"]) + 54,
        "V3 validation seed range is not one paired matrix",
    )
    _require(int(matrix["max_ticks"]) == 30_000, "unexpected V3 horizon")
    _require(
        matrix["paired_models_share_identical_task_seeds"] is True,
        "V3 validation matrix is not paired",
    )
    _require(
...
def materialize(
    ...
    completion_path = Path(str(target["expected_completion_path"])).resolve(
        strict=True
    )
    ...
    _require(
        ...
        "settled-V3 distillation is not complete",
    )
    checkpoint_by_epoch = {
        int(row["epoch"]): row
        for row in completion["optimization"]["checkpoints"]
    }
    _require(
        set(checkpoint_by_epoch) == {2, 4, 6, 8},
        "completion checkpoint set differs from the frozen plan",
    )
    models = [
        {
            "id": str(plan["models"]["baseline"]["id"]),
            "training_steps": int(
                plan["models"]["baseline"]["training_steps"]
            ),
            "path": str(
                Path(str(plan["models"]["baseline"]["path"])).resolve(
                    strict=True
                )
            ),
            "sha256": str(plan["models"]["baseline"]["sha256"]),
        }
    ]
    for expected in target["expected_checkpoints"]:
        ...
        _require(
            ...
            f"epoch {epoch} checkpoint path differs from plan",
        )
    ...
    for path in (manifest_path, preregistration_path, run_root):
        _require(not path.exists(), f"V3 validation output already exists: {path}")

    master_path = Path(str(plan["master_preregistration"]["path"])).resolve(
        strict=True
    )
    evaluator_path = Path(
        str(plan["implementation"]["evaluator"]["path"])
    ).resolve(strict=True)
    ...
    manifest["settled_v3_validation_plan"] = plan_reference
    manifest["target_completion"] = completion_reference
    preregistration["settled_v3_validation_plan"] = plan_reference
    preregistration["target_completion"] = completion_reference
    _require(
        preregistration["seed_plan"]["base_seed"]
        == plan["matrix"]["base_seed"]
        and preregistration["seed_plan"]["last_seed"]
        == plan["matrix"]["last_seed"],
        "materialized seed matrix differs from plan",
    )
    _require(
        preregistration["evaluator"]["sha256"]
        == plan["implementation"]["evaluator"]["sha256"],
        "materialized evaluator differs from plan",
    )
