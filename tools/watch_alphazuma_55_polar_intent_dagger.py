"""Wait for raw-intent completion, then run its frozen DAgger follow-up."""
...
from tools.materialize_alphazuma_55_polar_intent_dagger import (
    materialize,
...
def _training_snapshot(run_dir: Path) -> dict[str, Any] | None:
    ...
    result = {
        ...
        "completed_epochs_in_round": value.get(
            "completed_epochs_in_round"
        ),
...
def run(
    *,
    ...
    if (
        ...
        raise ValueError("plan binds another raw-intent DAgger watcher")
    status_root = Path(str(plan["outputs"]["status_root"])).resolve()
    if status_root.exists():
        raise FileExistsError(
            f"raw-intent DAgger watcher root exists: {status_root}"
        )
    ...
    def status(stage: str, **extra: Any) -> None:
        _write_atomic(
            ...
            {
                "schema": (
                    "zuma-rl.alphazuma-55-polar-intent-dagger-watcher"
                ),
                "version": 1,
                "status": "RUNNING",
                "stage": stage,
    ...
    try:
        while not completion_path.exists():
            if source_failure_path.exists():
                raise RuntimeError(
                    "raw-intent source training failed: "
                    + json.dumps(_read(source_failure_path), ensure_ascii=False)
                )
            if not _pid_alive(source_controller_pid):
                raise RuntimeError(
                    "raw-intent source controller exited without a terminal "
                    "receipt"
                )
            status("WAITING_FOR_RAW_INTENT_SOURCE")
        ...
        with (status_root / "trainer.stdout.log").open(
            ...
            if process.returncode != 0:
                ...
                raise RuntimeError(
                    "raw-intent DAgger trainer exited with code "
        ...
        if not completion.exists():
            raise RuntimeError(
                "raw-intent DAgger trainer omitted completion receipt"
            )
        ...
        if not (
            ...
            raise RuntimeError(
                "raw-intent DAgger completion is not eligible"
            )
        final = {
            "schema": "zuma-rl.alphazuma-55-polar-intent-dagger-watcher",
            "version": 1,
            "status": "COMPLETE",
            "stage": "COMPLETE",
        ...
        failure = {
            "schema": (
                "zuma-rl.alphazuma-55-polar-intent-dagger-watcher-failure"
            ),
            "version": 1,
            "status": "ERROR",
            "stage": "ERROR",
