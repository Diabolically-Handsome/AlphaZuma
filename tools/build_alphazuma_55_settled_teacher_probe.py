"""Freeze the fresh 55-level probe for the aim-settled teacher."""
...
EVALUATOR = PROJECT_ROOT / "tools/evaluate_alphazuma_55_settled_teacher.py"
SETTLED_TEACHER = PROJECT_ROOT / "src/zuma_rl/revenge_settled_strategic_teacher.py"
...
def build(*, output_root: Path, workers: int) -> dict[str, Any]:
    ...
    if not (
        ...
        raise ValueError("settled probe seeds escaped engineering namespace")
    return {
        "schema": "zuma-rl.alphazuma-55-settled-teacher-probe-preregistration",
        "version": 1,
        "status": "FROZEN_BEFORE_FRESH_ENGINEERING_PROBE",
        "created_utc": datetime.now(timezone.utc).isoformat(),
        "campaign_id": master["campaign_id"],
        "objective": (
            "Measure fresh 55-level coverage after gating strategic fire edges "
            "until the actor-observable cursor is within two aim bins."
        ),
        ...
        "development_disclosure": {
            ...
            "calibration_seeds_already_consumed": [
                1400700100,
                1400500022,
                1400500029,
                1400500039,
            ],
            "calibration_levels": ["Jungle1", "city5", "Coast3", "grotto4"],
            "fresh_seed_relative_to_frozen_teacher_bytes": True,
        ...
        "implementation": {
            ...
            "settled_teacher": legacy._artifact(SETTLED_TEACHER),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if output_root.exists():
        raise FileExistsError(f"settled teacher output root exists: {output_root}")
